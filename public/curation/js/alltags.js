...
window.TagView = Backbone.View.extend({  
	...
	window.ViewNum = Backbone.View.extend({  
	tagName:  "li",
	template: _.template($('#tag-template').html()),      
	render: function() {	
		$(this.el).html(this.template(this.model.toJSON()));     
...
function getAllTags(){
	...
	if($("#isPublic").html() == "false"){
		data = { 'fq':'author:'+ $("#username").html() };		
	}else{
	...
	$.ajax({
		...
		success: function(data) {
			...
			Tags.reset(tagTemp);
			var tagnum =  new ItemList();
			tagnum.reset(tagTemp);
			...
			tagnum.each(function(item){			
				if(item != undefined){
					var viewNum = new ViewNum({model:item});
					$("#tag-num").append(viewNum.render().el);
				}
			});			
			setTagCloudStyle();
		}
...
function jumpToTag(path){
	...
	location = "/searchpage" + path + "/tag/\"" + $("#Inputs").val()+"\"";
}
